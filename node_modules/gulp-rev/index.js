...
plugin.manifest = (pth, opts) => {
	...
	return through.obj((file, enc, cb) => {
		...
		const originalFile = opts.pwd_base + '/' + path.join(path.dirname(revisionedFile), path.basename(file.revOrigPath)).replace(/\\/g, '/');

		// manifest[originalFile] = revisionedFile;// 原文
		//originalFile = opts.pwd_base + '/' + originalFile; //修改处
		manifest[originalFile] = originalFile + '?v=' + file.revHash;// 修改处
